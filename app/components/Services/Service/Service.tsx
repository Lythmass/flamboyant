...
export const Service: React.FC<ServiceType> = (props) => {
  ...
  return (
    <div
      ...
      className={`w-full min-h-[27rem] p-10 flex flex-col gap-1 ${isOddAndLast}`}
    >
      <div className='w-full flex flex-col-reverse lg:flex-row lg:items-center gap-7 lg:gap-0 lg:justify-between'>
        ...
        <div className={`${props.animatedIcon} relative`} />
      </div>
      <p
        className={`${fontSecondary.className} font-medium text-lg pr-10 text-dark`}
      >
        {props.shortDescription}
      </p>
      <ul
        className={`${fontSecondary.className} text-dark text-2xl flex flex-col gap-1 mt-7 font-medium`}
      >
        {displaySubServices}
      </ul>
    </div>
